...
class ObjectFind:
    """ObjectFind class:
    A class that instantiates and starts looking for specified objects. 
    Contains state variables describing the locations of the requested objects, and functions to analyze video data and relocate Edwin to directly above a requested object.
    | see | | terminal output describing running processes
    """

    def __init__(self, init=False):
        """__init__ function
        Run once when a new ItemFind instance is created, and initializes variables to store object locations and other information. 
        Also starts the publisher and subscriber nodes necessary to run Edwin.
        """
    ...
    def apply_filter(self, frame):
        """Applies filter to find contours in input video frame. Called by calibrate function.
        Input: Video frame 'self.cvimage', created by callback function from the ROS image published by the cameras.
        Output: Returns video frame '' with contours.
        """

        #Checks to see if there is an input video frame.
        if feed == None:
            print "No video feed."
            return

        blur = cv2.GaussianBlur(feed, (5,5), 0) # Applies a Gaussian Blur filter

        # Calls function contour_feed to contour  and calculate moments
    ...
    def contour_feed(self, video):
        ...
        processed_img = cv2.morphologyEx(thresh,cv2.MORPH_OPEN,kernel, iterations = 2) #Erodes image, then dilates. Acts 2x, using 'kernel' as kernel.

        ## # Cleans out background through extra dilations (3x)
        ## sure_bg = cv2.dilate(opening,kernel,iterations=3)

        ## blurred = cv2.GaussianBlur(hsv, (21, 21), 0)
        ## (cnts, _) = cv2.findContours(cv2.GaussianBlur(mask.copy(), (25,25), 0), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE) # Calculates contours

        (cnts, _) = cv2.findContours(processed_img, cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE) # Calculates contours
        objects = []
        ...
        for c in cnts:
            ...
            ## Find Center of Contour Moments
            M = cv2.moments(c)
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            ...
            ## Protect limits to prevent overflow
            if (cX >= 640):
                    cX = 640 
            elif (cX <= 0):
                    cX = 1
        ...
        # finalcontours = [None]*3 # 1st Elem: Object | 2nd Elem: Hand
        finalcontours = [None]
        for cnt in contours:
